...
class Planner:
    ...
    def search(self):
        ...
        if self.same_city:
            # 同城多日(如"重庆两日游")也需要酒店过夜: 遍历预算内酒店排多日
            if self.days > 1 and self.hotels:
                # **要在预算内取"花得最充分"的那个**，不能拿到第一个完整方案就返回。
                # 跨城分支一直是 `cost > best_cost` 择优，同城这条漏了 —— 后果是：
                # 用户给了 4000 的预算，排出来只花 1296（实测"重庆有什么好玩的推荐预算4000两个人"），
                # 用户会直接问"花费是不是太少了"。注释里写的"取预算内花费最充分的一个"对同城不成立。
                best, best_cost = None, -1.0
                for hotel in self._hotel_order():
                    if self._capped():
                        break
                    plan, cost, complete = self._trip(None, None, hotel)
                    if plan and complete and cost <= self.budget * (1 + BUDGET_TOLERANCE):
                        if cost > best_cost:
                            best, best_cost = plan, cost
                if best:
                    return True, best
                # 多日排不出(通常是酒店数据不足), 落到下面回退单日, 保证至少有结果
