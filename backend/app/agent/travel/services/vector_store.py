...
#知识库分批写库的批大小：扩到 3.4 万块后，一次性 add_documents 内存峰值过高
KNOWLEDGE_ADD_BATCH = 2000
...
class VectorStoreService:
    ...
    # where: Chroma 的 metadata 过滤条件, 如 {"title": {"$in": ["重庆"]}} —— 用来把检索限定在城市范围内
    def search_with_score(self, query: str, k: int | None = None, where: dict | None = None):
        ...
        for path in allowed_file_path:
            ...
            try:
                ...
                if not final_documents:
                    ...
                    continue
                #分批写库：知识库扩到 3.4 万块后，一次性 add 会把文本和向量同时压进内存
                for i in range(0, len(final_documents), KNOWLEDGE_ADD_BATCH):
                    self.vector_store.add_documents(
                        final_documents[i:i + KNOWLEDGE_ADD_BATCH]
                    )
                #保存md5, 避免重复加载
                self._save_md5_hex(md5_hex)
                logger.info(f"[加载知识库] {path}内容已保存({len(final_documents)}块)")
