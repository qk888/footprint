...
# override=True: 避免容器 env_file 注入的空值挡住 .env 文件里的真值
...
class RollingGoClient:
    ...
    def _fallback(self, city: str) -> list[dict]:
        ...
        为什么不止一家：多天行程每天要换/续酒店，只有一家会让引擎没得选。
        坐标故意留 0 —— 由 `travel_plan_tool.load_data` 用同城 POI 质心兜住
        （上游一挂就返回 (0,0) 的话，酒店在几内亚湾，引擎算市内交通要几万分钟，
        每个组合都失败，用户看到的是莫名其妙的"无法规划"）。
        estimated=True 让卡片能如实说明"这是估算价"。
        """
        tiers = [(f"{city}快捷酒店", 180), (f"{city}商务酒店", 280), (f"{city}精品酒店", 420),
                 (f"{city}公寓式酒店", 320), (f"{city}度假酒店", 600)]
        return [
            {"name": name, "city": city, "price": float(price),
             "lat": 0, "lon": 0, "numbed": "双人床", "estimated": True}
