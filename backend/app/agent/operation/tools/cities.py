...
from app.agent.constraints import write_intent_ok, WRITE_DENIED_MSG
...
def city_tools(service: CitiesService, user_id: int):
    ...
    async def light_city(city_names: list[str]) -> str:
        if not city_names:                                     # 空列表防御: 没有要点的城市
            return "没有需要点亮的城市"
        # 护栏：**模型不得凭空点亮城市**。实测 query="4" 时被判成 operation，
        # 中间件 force_first_tool_call 逼着"首轮必须调工具"，模型没内容可调就从
        # 上下文/记忆里编了 city_names=['北京','上海'] 真去写库 —— 用户根本没提点亮。
        # 判定规则在 harness 约束层（constraints.WRITE_RULES["light_city"]），
        # 中间件 guard_write_tools 已经拦过一道；这里再拦一道是**防御纵深**：
        # 万一这个工具被挂到没带闸门的 agent 上，也不会变成数据事故。
        allowed, why = write_intent_ok("light_city")
        if not allowed:
            logger.warning(f"[light_city] 拒绝执行：{why} 请求={city_names}")
