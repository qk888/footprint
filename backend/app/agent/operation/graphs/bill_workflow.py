    ...
    async def _add_bill(self, state: BillState):               # 步骤3: 真正写账单(循环写多笔)
        ...
        try:
            try:
                ...
                # 城市已点亮但没有旅行(历史数据/旅行被删), 补建一个再继续
                if e.status_code == 400 and "暂无旅行" in str(e.detail):
                    logger.info(f"[AutoBill] {state['city_name']}无旅行, 自动补建")
                    trip = await self.trip_svc.add_trip(state["adcode"], state["user_id"])
                else:
                    raise
