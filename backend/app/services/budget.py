...
from app.services.memories import category_label, city_name, replace_event
...
class BudgetService:
    ...
    async def set_budget(self, user_id: int, adcode: str, category: int, amount, custom_category: str | None = None):
        ...
        result = await self.repo.set_budget(user_id=user_id, adcode=adcode, category=category, amount=amount, custom_category=custom_category)
        #金额会被改, 先按开头清掉旧记忆再写新的
        prefix = f"给{city_name(adcode)}的{category_label(category, custom_category)}设了"
        await replace_event(self.repo.session, user_id, "budget", prefix, f"{prefix}{amount}元预算")
        return result
