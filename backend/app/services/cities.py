...
from app.services.memories import city_name, forget_event, remember_event

#点亮记忆的说法: 写入和撤销必须用同一句话, 否则撤销不掉
def light_memory_text(adcode: str):
    return f"去过{city_name(adcode)}"
...
class CitiesService:
    ...
    async def light_city(self, user_id: int, adcode: str):
        ...
        await remember_event(self.city_repo.session, user_id, "city_light", light_memory_text(adcode))
        return result
    ...
    async def unlight_city(self, user_id: int, adcode: str):
        ...
        await forget_event(self.city_repo.session, user_id, "city_light", light_memory_text(adcode))
        return result