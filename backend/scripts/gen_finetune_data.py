...
from app.agent.footprint_agent import build_supervisor_tools
...
OPERATION_TOOLS = (
    ...
    + [_to_openai(t) for t in memory_tools(None, 1)]
    + [_to_openai(out_of_scope)]
)
...
def build_examples():
    ...
    # ])
    # # operation 弃权：误路由时不许硬调业务工具(否则真会点亮城市/写账单)
    # add(OPERATION_PROMPT, OPERATION_TOOLS, [
    #     H("重庆旅游两日计划"),
    #     CALL("out_of_scope"),
    #     O("这个问题不归我管"),
