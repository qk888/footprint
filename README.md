...
<br/>
<img src="docs/screenshots/logo.png" width="96" height="96" alt="足迹 Footprint Logo"/>
...
**旅行足迹地图 + 行程规划 + AI 攻略问答三合一的 Web 应用**

在地图上点亮去过的城市、一句话记账；AI 助手听懂一句"上海去杭州玩4天，预算3000"，
直接生成**可执行**的时间轴行程单——几点坐什么车、在哪吃饭、住哪、每项花多少钱。

<div align="center">
  <img src="docs/screenshots/02_map.png" alt="足迹地图：点亮的城市高亮渲染，看全球用 Esri 世界底图，放大进中国自动切高德中文" width="100%"/>
  <p><sub>足迹地图 —— 点亮的城市高亮渲染；AI 生成的行程可直接落到对应城市</sub></p>
</div>

## 界面预览

| AI 助手 · 时间轴行程卡片 | 行程规划 · 多份方案切换 |
|---|---|
| <img src="docs/screenshots/03_chat.png" alt="AI 助手：一句话生成三亚三天行程"/> 一句话生成行程：几点出发、在哪吃饭、住哪、每项花多少钱；还按「不吃辣」的偏好排除了辣味餐厅 | <img src="docs/screenshots/04_plans.png" alt="行程规划：同一城市可保存多份方案"/> 同一城市可保存多份方案，顶部横向切换、另存为新规划 |
| **旅行记录 · 账单明细** | **AI 记忆 · 待确认机制** |
| <img src="docs/screenshots/05_records.png" alt="旅行记录：按行程展开的账单明细"/> 按行程归档的账单明细，点开即看每一笔 | <img src="docs/screenshots/06_profile.png" alt="我的：AI 记忆列表"/> AI 记住你的偏好（我不吃辣 / 讨厌排队），**待确认的不擅自生效**，确认后才进规划 |

> 登录页与更多界面见 [`docs/screenshots/`](docs/screenshots/)（含 [登录页](docs/screenshots/01_login.png)）。

## 作者

**青** —— 品牌与界面设计、足迹地图与点亮、行程规划与偏好联动、知识库检索增强、联网兜底与自动化测试。

许可：PolyForm Noncommercial 1.0.0（个人学习 / 研究等非商业用途），详见 [LICENSE](LICENSE) 与 [NOTICE](NOTICE)。
...
- 底图按视口自动切换：看全球用 Esri 世界底图（境外国名城市齐全），
  放大进中国自动切高德中文（街道级细节），另有 Esri 卫星可选

### 🧭 AI 行程规划
- 小模型槽位填充：一句话提取出发地 / 目的地 / 天数 / 预算（RAG 少样本示例增强）；
  模型漏填时按"从A到B"句式正则兜底，省名自动归一到城市（"云南五天"→昆明）
- 确定性 DFS 规划引擎：时间驱动状态机 + 回溯剪枝 + 动态预算拆分，
  基于真实景点 / 车次数据规划，杜绝大模型"编车次、算错账"
- 输出按天分组的时间轴行程卡片：交通 / 餐饮 / 住宿 / 景点 + 逐项费用 + 超支预警
- **记忆偏好进规划**：确认过的"我不吃辣 / 海鲜过敏 / 讨厌排队"会真实影响结果——
  辣味餐厅剔除、过敏源硬过滤、排队敏感的景点降权，卡片上注明生效了哪些偏好
- 景区别名识别："香格里拉 / 九寨沟 / 婺源"自动映射到所属城市，并向你说明
...
- 检索不到或跨出能力边界时如实说明；跨境线路给联网检索到的原始资料条目（注明未核实），
  不替模型"编一个看起来可行的答案"

### 🛡️ 对话稳定（Harness 架构）
- 确定性 Python 代码对模型输出验收重试：合格直出、失败摘除子智能体重开对话
- 写操作三层约束：用户原话授权 → 写入取证台账 → 回复声称校验
  （模型声称"已点亮"但查不到记录时，自动换成如实答复）
...
```mermaid
flowchart LR
    A[Web 前端 Vue 3] -->|HTTP / SSE| B[FastAPI 后端]
    B --> C{Supervisor 意图路由}
    C -->|旅行| D[travel 子智能体]
...
| 层 | 技术 |
|---|---|
| 前端 | Vue 3 + TypeScript + Vite + naive-ui + Leaflet（高德 / Esri 底图，含 AMap JS API） |
...
│   │   ├── data/                   # 运行时数据缓存(不随仓库分发)
│   │   └── agent/                  # AI 多智能体
│   │       ├── footprint_agent.py  # harness 主循环 + 直达通道 + 意图分类
│   │       ├── middleware.py       # harness 中间件(结果验收 / 收口 / 写操作闸门)
│   │       ├── constraints.py      # harness 约束层(写操作授权 / 取证 / 声称校验)
│   │       ├── prompts/            # 各角色提示词
│   │       ├── config/             # yml 配置(模型 / 向量库)
│   │       ├── data/               # 槽位示例 + 攻略语料(语料不入库, 可由脚本重建)
│   │       ├── model/              # 模型工厂(llama.cpp 接入)
│   │       ├── chat/               # 闲聊子智能体
│   │       ├── operation/          # 软件操作子智能体(记账 / 城市 / 记忆)
│   │       └── travel/             # 旅行子智能体
│   │           ├── services/       # 规划引擎(planner_engine) / RAG / 槽位填充
│   │           ├── tools/          # travel_plan / rag 工具
│   │           └── data_source/    # 高德 / 12306 / RollingGo 数据源
│   ├── train_data/                 # 微调训练数据(tool_calling_train.jsonl 等)
│   └── scripts/                    # 工具脚本(向量库构建 / 微调数据 / 回归测试)
├── finetune/                       # LLaMA-Factory 微调配置
└── web/                            # Vue 3 Web 前端
...
MySQL 中创建 `footprint` 库（字符集 utf8mb4），然后执行迁移：
...
- 向量库生成在 `backend/app/agent/chroma_db/`，支持增量更新（按文件 md5 去重，台账在 `chroma_db/md5.text`），重复执行安全
- **仓库不包含攻略语料**：`backend/app/agent/data/*.txt` 被 .gitignore 挡在外面（15MB，且是可由脚本重建的产物）。
  clone 之后想有攻略数据，先生成语料再执行上面的 ②：

```bash
# 下载中文维基导游 dump 并切块（默认输出到 app/agent/data/，约 15MB / 34313 块）
python scripts/build_wikivoyage_kb.py
# 也可以换成自己的资料：往 app/agent/data/ 放 .txt（按城市组织）/ .pdf / .csv 即可
```
...
生产构建走容器：`docker compose --profile web up -d frontend`（nginx 反代，<http://localhost:8080>）。
...
依次检查：MySQL 是否启动、`footprint` 库是否已建、`alembic upgrade head` 是否执行、
...
分发时请保留本许可协议与作者署名；使用第三方 API 拉取的数据请遵守数据源服务条款，不得再分发。
